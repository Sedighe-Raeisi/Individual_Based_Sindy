...
class LotkaVolterra:
    ...
    def simulate(self, t_span=(0, 20), dt=0.01, noise_std=0.1):
        """
        ...
        if self.gradient_targets:
            ...
            dv_dt = self.delta * results[0] * results[1] - self.gamma * results[1]
...
print("---------------------------- Mix Function -------------------------")
def mix_data_lotka_volterra(system_param_dict):
    ...
    noise_std = system_param_dict['noise_info']['noise_std'] if 'noise_std' in system_param_dict['noise_info'] else 0.01
    ...
    # if alpha_V:
    #     alpha_vals = np.array([alpha_V])
    # else:
    #     alpha_vals = np.abs(np.random.normal(alpha_mean, alpha_std, alpha_N))
    #
    # if beta_V:
    #     beta_vals = np.array([beta_V])
    # else:
    #     beta_vals = np.abs(np.random.normal(beta_mean, beta_std, beta_N))
    #
    # if gamma_V:
    #     gamma_vals = np.array([gamma_V])
    # else:
    #     gamma_vals = np.abs(np.random.normal(gamma_mean, gamma_std, gamma_N))
    #
    # if delta_V:
    #     delta_vals = np.array([delta_V])
    # else:
    #     delta_vals = np.abs(np.random.normal(delta_mean, delta_std, delta_N))
    #
    # param_sets = []
