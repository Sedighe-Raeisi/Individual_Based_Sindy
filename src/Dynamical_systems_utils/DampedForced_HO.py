...
class HarmonicOscillator:
    ...
    def simulate(self, t_span=(0, 10), dt=0.01, noise_std=0.01):
        ...
        # true_dvdt = -self.k / self.m * results[0] - self.c / self.m * results[1] + self.F0 / self.m * np.cos(self.omega * t_eval)
        # true_dxdt = results[1]
        #
        # dx_dt = true_dxdt + np.random.normal(0, noise_std, len(t_eval))
        # dv_dt = true_dvdt + np.random.normal(0, noise_std, len(t_eval))
...
def mix_data(system_param_dict):
    ...
    Args:
        ...
        noise_std (float, optional): Standard deviation of noise added to data. Defaults to 0.01.
    ...
    #     m_vals = np.array([m_V])
    # else:
    #     m_vals = np.abs(np.random.normal(m_mean, m_std, m_N))
    #
    # if k_V is not None:
    #     k_vals = np.array([k_V])
    # else:
    #     k_vals = np.abs(np.random.normal(k_mean, k_std, k_N))
    #
    # if c_V is not None:
    #     c_vals = np.array([c_V])
    # else:
    ...
    #
    # if F0_V is not None:
    #     F0_vals = np.array([F0_V])
    # else:
    #     F0_vals = np.abs(np.random.normal(F0_mean, F0_std, F0_N))
    #     if F_zero_portion is not None:
    #         F0_vals_zero = np.zeros(int(F_zero_portion * F0_N))
    #         F0_vals = np.concatenate((F0_vals_zero,np.abs(np.random.normal(F0_mean, F0_std, F0_N - int(F_zero_portion * F0_N)))))
    #         # np.random.shuffle(F0_vals) # Removed: np.random.shuffle shuffles in-place and returns None
    #
    #
    # if omega_V is not None:
    #     omega_vals = np.array([omega_V])
    # else:
    #     omega_vals = np.abs(np.random.normal(omega_mean, omega_std, omega_N))
    #
    # param_sets = []
    # for m in m_vals:
    #     for k in k_vals:
    ...
    for param_i in range(N_param_set):
        ...
        results = oscillator.simulate(t_span=(t_start, t_end), dt=dt, noise_std=noise_std)
