...
def mix_data_LV_M(system_param_dict):
    ...
    for _ in range(N_param_set):
        ...
        # dxdt = alpha * x - alpha * x**2 - (beta * x * y) / (1 + h * x)
        ...
        X = np.array([
            ...
            x * y**2,  # 7. f2(x)*Y = (beta*x*y)/(1+h*x)
            dydt,
            x*dydt
        ])
        Y_target = np.array([np.zeros_like(dydt), np.zeros_like(dydt)])
...
def gt_utils(real_params):
    ...
    gt_coef_dx["x"] = get_stats(-1*real_params['alpha_array'])

    coef_x2 = -real_params["alpha_array"]*(real_params["h_array"]-1)
    gt_coef_dx["x**2"] = [np.mean(coef_x2), np.std(coef_x2)]

    coef_x3 = real_params["alpha_array"]*real_params["h_array"]
    gt_coef_dx["x**3"] = [np.mean(coef_x3),np.std(coef_x3)]

    gt_coef_dx["x_dot"] = [1,0]

    gt_coef_dx["x_dot*x"] = [np.mean(real_params["h_array"]),np.std(real_params["h_array"])]

    gt_coef_dx["x*y"] = [np.mean(real_params["beta_array"]),np.std(real_params["beta_array"])]
    ################################

    gt_coef_dy = {name: [0, 0] for name in coef_names}

    gt_coef_dy["y"] = [np.mean(real_params["m_array"]),np.std(real_params["m_array"])]
    ...
    gt_coef_dy["x*y**2"] = [np.mean(coef_xy2),np.std(coef_xy2)]

    gt_coef_dy["y_dot"] = [1,0]

    gt_coef_dy["x*y_dot"] = [np.mean(real_params["h_array"]),np.std(real_params["h_array"])]
...
def generate_pdf(save_path, pdf_smaple_N=10000, epsilon=0.01):
    ...
    coef_0 = [
        ...
        -p['alpha']*(p["h"]-1),  # 2. x**2
        p["alpha"]*p["h"],  # 3. h**3)
        np.random.normal(1, epsilon, pdf_smaple_N),  # 4. x_dot
        p["h"],  # 5. x_dot*x
        noise_vec,  # 6. y
        noise_vec,  # 7. y**2
        p['beta'],  # 8. x*y
    ...
    coef_1 = [
        ...
        p["m"],  # 6. y
        p["H"],  # 7. y**2
        p["m"]*p["h"]-p["epsilon"]*p["beta"],  # 8. x*y
        p["H"]*p["h"],  # 9. x*y**2
        np.random.normal(1, epsilon, pdf_smaple_N),  # 10 y_dot
        p["h"]  # 11. x*y_dot
    ]
