...
class FitzHughNagumo:
    ...
    def simulate(self, t_span=(0, 10), dt=0.01, noise_level=0.01):
        ...
        vnoise_scale = np.std(results[0]) * noise_level
        wnoise_scale = np.std(results[1]) * noise_level

        v = results[0] + np.random.normal(0, vnoise_scale, len(t_eval))
        w = results[1] + np.random.normal(0, wnoise_scale, len(t_eval))
...
def mix_data(system_param_dict):
    ...
    for param_i in range(N_param_set):
        ...
        results = fhn_system.simulate(t_span=(t_start, t_end), dt=dt, noise_level=noise_level)
